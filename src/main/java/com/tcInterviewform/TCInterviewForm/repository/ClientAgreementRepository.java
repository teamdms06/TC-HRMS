///*
// * To change this license header, choose License Headers in Project Properties.
// * To change this template file, choose Tools | Templates
// * and open the template in the editor.
// */
//package com.tcInterviewform.TCInterviewForm.repository;
//
//import com.tcInterviewform.TCInterviewForm.model.ClientAgreement;
//import java.util.List;
//import org.springframework.data.jpa.repository.JpaRepository;
//import org.springframework.data.jpa.repository.Query;
//import org.springframework.stereotype.Repository;
//
///**
// *
//// * @author teamd
// */
//@Repository
//public interface ClientAgreementRepository extends JpaRepository<ClientAgreement, Integer> {
//
//    @Query(value = "SELECT * FROM `client_agreement` WHERE STATUS='ACTIVE'", nativeQuery = true)
//    public List getClientList();
//
//    @Query(value = "SELECT * FROM `client_agreement` WHERE id=?1", nativeQuery = true)
//    public List getAllAgreement(Integer id);
//
//}
